...
countOccurrences <- function(x,
  ...
  x <- x %>%
    addIntersect(
      cdm = cdm, tableName = tableName, filterVariable = filterVariable,
      filterId = cohortId, idName = idName, value = "count", indexDate = indexDate,
      targetStartDate = targetStartDate, targetEndDate = targetEndDate,
      window = window, nameStyle = nameStyle, tablePrefix = tablePrefix
    )
