...
We can use table intersection functions to check whether someone had a record in the drug exposure table in the 30 days before their ankle sprain. With overlap as TRUE we are checking that they had an ongoing drug exposure record in the window.
...
Meanwhile if we set overlap as FALSE we are checking that they had a drug exposure record that started during the window.
...
cdm$acetaminophen_records <- cdm$drug_exposure |> 
  filter(drug_concept_id %in% acetaminophen_cs$acetaminophen) |> 
  compute()
