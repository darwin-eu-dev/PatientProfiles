...
In this vignette we introduce the `targetEndDate` functionality found in `addCohortIntersectCount()` and `addCohortIntersectFlag()` functions. In both of them, there are three reference dates which can be specified:
...
As for the functions `addCohortIntersectTime()` and `addCohortIntersectDate()`, they need a specific date in the target cohort to calculate time outputs. Therefore, only `targetDate` needs to be specified, which is set to "cohort_start_date" by default.
