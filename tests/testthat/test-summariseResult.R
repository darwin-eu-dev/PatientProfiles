...
test_that("obscure", {
  ...
  expect_true(nrow(s) == 21)
  expect_true(sum(s$estimate[!is.na(s$estimate)] == "<2") == 1)
  expect_true(sum(is.na(s$estimate)) == 1)

  # minCellCount = 3
  s <- summariseResult(x, minCellCount = 3)
  expect_true(nrow(s) == 21)
  expect_true(sum(s$estimate[!is.na(s$estimate)] == "<3") == 3)
  expect_true(sum(is.na(s$estimate)) == 3)

  # minCellCount = 4
  s <- summariseResult(x, minCellCount = 4)
  expect_true(nrow(s) == 21)
  expect_true(sum(s$estimate[!is.na(s$estimate)] == "<4") == 5)
  expect_true(sum(is.na(s$estimate)) == 5)

  # minCellCount = 5
  s <- summariseResult(x, minCellCount = 5)
  expect_true(nrow(s) == 21)
  expect_true(sum(s$estimate[!is.na(s$estimate)] == "<5") == 7)
  expect_true(sum(is.na(s$estimate)) == 7)

  # minCellCount = 6
  s <- summariseResult(x, minCellCount = 6)
  expect_true(nrow(s) == 21)
  expect_true(sum(s$estimate[!is.na(s$estimate)] == "<6") == 7)
  expect_true(sum(is.na(s$estimate)) == 7)
  ...
  expect_true(sum(is.na(s$estimate)) == 20)
})
