...
test_that("addInObservation, cohort and condition_occurrence", {
  ...
  expect_true("flag" %in% colnames(result1))
  expect_true(all(result1 %>% dplyr::arrange(cohort_definition_id, cohort_start_date) %>% dplyr::select(flag) %>% dplyr::pull() == c(1,1,1,0)))
  result2 <- addInObservation(cdm$cohort2,cdm)


  expect_true("flag" %in% colnames(result2))
  expect_true(all(result2 %>% dplyr::arrange(cohort_definition_id, cohort_start_date) %>% dplyr::select(flag) %>% dplyr::pull() == c(1,0,1,1,1)))

  result3 <- addInObservation(cdm$cohort1 %>% dplyr::rename(person_id = subject_id),cdm)
  expect_true("flag" %in% colnames(result3))
  expect_true(all(result1 %>% dplyr::select(flag) %>% dplyr::pull() == result3 %>% dplyr::select(flag) %>% dplyr::pull()))
